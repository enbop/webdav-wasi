    ...
    B: WebDavBackend,
{
    fn open<'a>(
        ...
        async move {
            ...
            match metadata {
                Ok(existing) => {
                    if options.truncate && (options.write || options.append) {
                        backend.truncate(&normalized, 0).await.map_err(map_error)?;
                    }

                    let position = if options.append { existing.len } else { 0 };
                    return Ok(Box::new(DavFileImpl {
                        path: normalized,
                        backend,
                        position,
                ...
                Err(BackendError::NotFound { .. }) if options.write && options.create => {
                    backend
