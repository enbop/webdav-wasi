...
impl WebDavBackend for MemoryBackend {
    ...
    async fn truncate(&self, path: &str, size: u64) -> Result<()> {
        let normalized = normalize_path(path)?;
        let mut state = self.inner.lock().expect("memory backend lock");
        truncate_file_locked(&mut state, &normalized, size)
    }
...
fn truncate_file_locked(state: &mut State, path: &str, size: u64) -> Result<()> {
    let size = usize::try_from(size).map_err(|_| BackendError::FileTooLarge)?;
    let node = state
        .entries
        .get_mut(path)
        .ok_or_else(|| BackendError::NotFound {
    ...
    node.metadata = Metadata::file(size as u64);
    Ok(())
}
